...
locals {
  ...
  argument = format("'%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'",
    var.avx_controller_public_ip, var.avx_controller_admin_email,
    ...
    var.aviatrix_customer_id, var.controller_version, var.avx_controller_name, var.resource_group_name
  )
}

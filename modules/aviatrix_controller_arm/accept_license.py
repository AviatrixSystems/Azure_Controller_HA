...
def cplt_accept_license():
    ...
    subprocess.Popen(
        [
            ...
            "accept",
            "--urn",
            "aviatrix-systems:aviatrix-copilot:avx-cplt-byol-01:latest",
        ],
        stdout=subprocess.PIPE,
    )


def cplt_get_license_details():
    # Get the details if Azure Marketplace image terms
    process = subprocess.Popen(
        [
            "az",
...
if __name__ == "__main__":
    ...
    cplt_details = cplt_get_license_details()
    if not cplt_details["accepted"]:
        cplt_accept_license()